       ...
       {reviews.map(review => {
        return (
          ...
          <div className="flex space-x-1">
            {[1, 2, 3, 4, 5].map((star) => (
                <svg key={star} className='w-6 cursor-pointe' viewBox="0 0 24 24" fill="#F24444">
                    <path 
                        ...
                        fill={ star <= review.rating ? "#F87171" :"#94a3b8" } />
                </svg>
            ))}
        </div>
...
{/* custom button for slick slider  */}
      ...
      <div  onClick={()=> handlePrev()} className={`rounded-l-full text-xl md:text-2xl lg:text-4xl px-3 py-1 cursor-pointer text-gray-700`}> <BsArrowLeft/> </div>
      <div onClick={() => handleNext()}   className={`rounded-r-full text-xl md:text-2xl lg:text-4xl px-3 py-1 cursor-pointer  text-gray-700`}> <BsArrowRight/> </div>
      </div>
    </div>
  );
}
